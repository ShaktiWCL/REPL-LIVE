...
{
    DefaultLayout = RDLC;
    RDLCLayout = './ReportLayout/SalesProformaInvoice.rdl';
    ...
    dataset
    {
        dataitem("Sales Header"; "Sales Header")
        {
            ...
            column(BilltoAddress_SalesHeader; "Sales Header"."Ship-to Address")
            {
            }
            column(BilltoAddress2_SalesHeader; "Sales Header"."Ship-to Address 2")
            {
            }
            column(BilltoCity_SalesHeader; "Sales Header"."Ship-to City")
            {
            }
            column(BilltoContact_SalesHeader; "Sales Header"."Ship-to Contact")
            {
            }
            ...
            dataitem("Sales Line"; "Sales Line")
            {
                ...
                column(TaxAmount_SalesLine; 0)
                {
                }
                column(TaxGroupCode_SalesLine; COPYSTR("Tax Group Code", 1, 3) + ' ' + 'FORMAT("Tax %")' + '%')
                {
                }
                ...
                begin
                    ...
                    // "Sales Header".CALCFIELDS("Amount to Customer");
                    // TotalAmount := ROUND("Sales Header"."Amount to Customer");
                    ...
                    IF SaleLine.FINDFIRST THEN BEGIN
                        ...
                        // TaxAmt += SaleLine."Tax Amount";
                        UNTIL SaleLine.NEXT = 0;
                    END;
                    TotalAmount := ROUND(Amt, 1) + ROUND(STaxAmt, 1) + ROUND(eCessAmt, 1) + ROUND(SHECessAmt, 1) + ROUND(SBCAmt, 1) + ROUND(KKCessAmt, 1) + ROUND(TaxAmt, 1);

                    Check.InitTextVariable;
                    Check.FormatNoText(AmountInWord, TotalAmount, "Sales Header"."Currency Code");
