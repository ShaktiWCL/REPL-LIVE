report 50099 "Pre Tax Invoice for RSPL"
{
    DefaultLayout = RDLC;
    RDLCLayout = './ReportLayout/PreTaxInvoiceforRSPL.rdl';
    ...
    dataset
    {
        dataitem("Sales Header"; "Sales Header")
        {
            ...
            column(ProjectName_1; ProjectName1)
            {
            }
            dataitem("Sales Line"; "Sales Line")
            {
                ...
                column(Unit_Price_SalesLine; "Unit Price")
                {
                }
                column(Tax_Amount_SalesLine; 0)
                {
                }
                column(TaxPer_SalesLine; 0)
                {
                }
                column(DiscountVal_SalesLine; DiscountVal)
                {
                }
                ...
                begin
                    ...
                    // ServiceTaxSetup.SETRANGE("From Date", 0D, "Posting Date");
                    // IF ServiceTaxSetup.FINDLAST THEN
                    //     TaxPer := ServiceTaxSetup."Service Tax %";

                    DiscountVal := 0;
                    DiscountVal := "Line Discount Amount" + "Inv. Discount Amount";

                    "Sales Header".CALCFIELDS(Amount);
                    Check.InitTextVariable;
                    Check.FormatNoText(AmountInWord, "Sales Header".Amount, "Sales Header"."Currency Code");
                end;
            }
            ...
            begin
                ...
                SalesCommentLine.RESET;
                SalesCommentLine.SETRANGE("No.", "No.");
                SalesCommentLine.SETFILTER("Line No.", '<>%1', 0);
                IF SalesCommentLine.FINDSET THEN
                    REPEAT
                        IF ProjectName1 <> '' THEN
                            ProjectName1 += ' ' + SalesCommentLine.Comment
                        ELSE
                            ProjectName1 := SalesCommentLine.Comment;
                    UNTIL SalesCommentLine.NEXT = 0;


                IF GetCurrency.GET("Currency Code") THEN
                    CurrencyCode := GetCurrency."Currency Symbol"
                ELSE
                    CurrencyCode := '';
            end;
